...
def tune_threshold_for_precision(y_true, prob_pos, min_precision=0.30):
    ...
    if len(thr) == 0:
        return 0.5, float(prec.mean()), float(rec.mean())
    ...
    i_local = int(np.argmax(rec[idxs]))
    i = idxs[i_local]
    return float(thr[i]), float(prec[i]), float(rec[i])
...
def train_one_run(csv_path: str, model_type: str, scaler_type: str, test_size: float, val_size: float, balance_delta: int, min_precision: float, random_state: int, base_log_dir: str):
    ...
    best_th, p_at, r_at = tune_threshold_for_precision(y_val, val_prob, min_precision=min_precision)
    log(f"Chosen threshold={best_th:.4f} (val precision≈{p_at:.3f}, recall≈{r_at:.3f})")
